...
def extractNodeTypes(filePath) :
	...
	def extractFamilyType(sectElem) :
		def findTypes() :
			...
			def getTypes(memberElem) :
				...
				def isTopNode(elem) :
					return definesNode(elem) and not hasAncestorElement(elem.parentNode, definesNode)
				...
				memberElem.getAttribute("linkend").find("node.") != 0 :
					...
					return map(getTitle, filter(isTopNode, descendantElements(sect)))
				return [textValue(memberElem)]
			rv = []
			for member in childElements(listElem) :
				rv += getTypes(member)
			return rv
	...
	setXmlId(doc)
	return map(extractNodeType, filter(definesNode, descendantElements(doc)))
