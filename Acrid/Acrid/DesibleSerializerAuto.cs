...
abstract class DesibleSerializerAuto : DesibleSerializerBase {
	...
	protected virtual XmlElement serialize(Node_Module node) {
		...
		append<Node_LimitOld>(elem, node.@limitOld, null);
		return elem;
	}
	...
	protected virtual XmlElement serialize(Node_Hidable node) {
		...
		append<INode_IdentikeySpecialNew>(elem, node.@declaration, "declaration");
		return elem;
	}
	...
	protected virtual XmlElement serialize(Node_LimitOld node) {
		XmlElement elem = _doc.CreateElement(node.typeName, desible1NS);
		append<INode_IdentikeySpecialOld>(elem, node.@declarations, "declaration");
	...
	protected virtual XmlElement serialize(Node_Compound node) {
		XmlElement elem = _doc.CreateElement(node.typeName, desible1NS);
		append<INode_IdentikeySpecialOld>(elem, node.@oldDeclarations, "old-declaration");
		append<INode_IdentikeySpecialNew>(elem, node.@newDeclarations, "new-declaration");
	...
	protected XmlElement serializeAny(INode node) {
		switch(node.typeName) {
			...
			case "limit-old":
				return serialize((Node_LimitOld)node);
			case "nullable-type":
