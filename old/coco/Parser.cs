...
public Bridge bridge;
	...
	void Expression(out INode_Expression expression) {
		...
		if (scanner.Peek().val == "(") {
			...
			ExtractNamedMember(out extract);
			expression = extract; 
	...
	void ExtractNamedMember(out Node_ExtractNamedMember extract) {
		INode_Expression expr;
		Node_Identifier ident; 
		SimpleExpression(out expr);
		ExpectLiteral(".", 19);
		Identifier(out ident);
		extract = new Node_ExtractNamedMember(expr, ident); 
	}
	...
	void String(out Node_String str) {
		Expect(7);
		str = new Node_String(bridge, t.val); 
	}
